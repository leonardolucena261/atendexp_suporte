...
<head>
 ...
 --txt-dark: #0F172A;
        ...
        * {
        ...
        .status-ativo {
            background: rgba(139, 189, 71, 0.12);
            color: #4d7c0f;
            border: 1px solid rgba(139, 189, 71, 0.2);
        }
...
<body>
    ...
    <main class="main-container">
        ...
        <div class="control-card">
            {{-- Bloco de Injeção do valor da URL para Paginação --}}
            @php
            ...
            <form method="GET" action="{{ route('aluno.index') }}">
                ...
                <div class="search-row">
                    <div class="search-input-wrap">
                        <input type="text" name="busca" id="busca" class="search-input"
                            placeholder="Nome, CPF ou RG do aluno..." value="{{ $buscaAtual }}"
                            aria-label="Buscar aluno por nome, cpf ou rg" autofocus>
            ...
            {{-- INÍCIO: Intercepta cliques de página e injeta a busca na URL dinamicamente --}}
            <script>
                document.addEventListener('DOMContentLoaded', function () {
                    ...
                    container.addEventListener('click', function (e) {
                        ...
                        // 1. Extrai base e query da URL atual
                        var href = link.getAttribute('href');
                        var urlObj;
                        var baseUrl = href.split('?')[0];
                        var queryString = href.split('?')[1] || '';

                        // 2. Valida se a base parece uma URL (evita erro em rotas como /aluno/{cod_turma})
                        try { urlObj = new URL(href); } catch (err) { }

                        var baseUrlFinal = urlObj ? urlObj.origin + urlObj.pathname : baseUrl;
                        var params = new URLSearchParams(queryString);

                        // 3. Pega o valor atual do input de busca
                        var inputBusca = document.getElementById('busca');
                        var buscaValor = inputBusca ? inputBusca.value.trim() : '';

                        // 4. Adiciona a busca no formato GET ou remove se estiver limpo
                        if (buscaValor) {
                            params.set('busca', buscaValor);
                        } else {
                            params.delete('busca');
                        }
                        ...
                        // 5. Pequeno atraso para garantir que navegadores modernos processem a troca
                        setTimeout(function () {
                ...
                <div class="aluno-card" id="card-{{ $aluno->cod_aluno }}">
                    ...
                    <div class="aluno-info">
                        ...
                        <div class="aluno-detalhes">
                            ...
                            @if($aluno->carteirinhaAtiva)
                                <span class="aluno-detalhe" style="color:#4d7c0f; font-weight:600;">
                                    <i class="fa-solid fa-id-badge" aria-hidden="true"></i>
                                    Carteira: {{ $aluno->carteirinhaAtiva->numero_carteirinha }}
                                </span>
                            @else
                                <span class="aluno-detalhe" style="color:#dc2626;">
                    ...
                    <div class="aluno-actions">
                        ...
                        @if($aluno->carteirinhas->count() > 0)
                            <button type="button" class="btn-action" onclick="toggleHistory(event, {{ $aluno->cod_aluno }})"
                                title="Historico de carteirinhas">
                                <i class="fa-solid fa-clock-rotate-left"></i> Historico ({{ $aluno->carteirinhas->count() }})
                            </button>
                        @endif

                        @if($aluno->carteirinhaAtiva)
                            <a href="{{ route('aluno.carteirinha', [$aluno->cod_aluno, $aluno->carteirinhaAtiva->id]) }}"
                                class="btn-action" title="Ver carteirinha">
                                <i class="fa-solid fa-print"></i> Ver
                            </a>
                            <button type="button" onclick="invalidarCarteira({{ $aluno->carteirinhaAtiva->id }})"
                                class="btn-action btn-danger" title="Invalidar esta carteirinha">
                                <i class="fa-solid fa-ban"></i> Invalidar
                            </button>
                        @else
                            {{-- Se a lógica for apenas gerar/ver a ativa, clique aqui --}}
                            <a href="{{ route('aluno.carteirinha', $aluno->cod_aluno) }}" class="btn-action"
                                title="Gerar/Ver carteirinha">
                                <i class="fa-solid fa-print"></i> Carteirinha
                            </a>
                        @endif
                    </div>
                </div>
    ...
    {{-- MENU DROPDOWN FLUTUANTE (Fora do container para nao ser cortado pelo overflow do card) --}}
    <div id="historyDropdown" class="history-dropdown" role="dialog" aria-label="Historico de carteirinhas">
        <div class="dropdown-header">
            <div class="dropdown-title"><i class="fa-solid fa-clock-rotate-left"
                    style="font-size:0.65rem; color:var(--txt-muted);"></i> Historico de Carteirinhas</div>
            <button class="dropdown-close" onclick="closeHistory()" title="Fechar" aria-label="Fechar historico">
                <i class="fa-solid fa-xmark"></i>
            </button>
        </div>
        <div class="dropdown-list" id="historyListContainer">
            {{-- O JS vai preencher isso dinamicamente --}}
        </div>
    </div>

    {{-- FORMULÁRIO MODAL PARA INVALIDAR --}}
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            var activeBtn = null; // Guarda referência do botão que abriu o dropdown
        ...
        /**
         * Abre o menu de histórico posicionado logo abaixo do botão clicado
         */
        function toggleHistory(event, codAluno) {
            event.stopPropagation(); // Impede que o clique feche o menu imediatamente
            closeHistory(); // Fecha qualquer outro aberto antes de abrir o novo

            var btn = event.currentTarget;
            activeBtn = btn; // Salva o botão atual para controle de clique fora
            ...
            // Preenche o conteúdo do dropdown (em produção, isso seria uma chamada AJAX)
            var historico = [
            ...
            // Se tivesse os dados reais do Laravel aqui via Blade, faria:
            // var historico = @json($aluno->carteirinhas->sortByDesc('created_at')->values());
        ...
        /**
         * Solicita o motivo e submete a invalidação
         */
